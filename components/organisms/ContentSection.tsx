...
import QuizLevelCard from "@/components/molecules/QuizLevelCard"
...
export default function ContentSection() {
  return (
    <section className="px-5">
      ...
      {/* FUNDAMENTAL */}
      <div id="fundamental-section" className="pt-24 pb-20 scroll-mt-24">
        ...
        <h2 className="text-center text-white font-bold text-[28px] sm:text-[40px] lg:text-[48px] leading-[1.15] mb-4 max-w-2xl mx-auto">
          Make yourself understand <br className="hidden sm:block" />the complex things with us
        </h2>
        ...
        <div className="flex justify-center gap-4 sm:gap-10 flex-wrap">
          {fundamentals.map((item) => (
            <Link key={item.label} href={item.href} className="group flex flex-col items-center gap-3 text-white w-[90px] sm:w-auto hover:scale-105 transition-all duration-200 active:scale-95">
      ...
      {/* EXPLAIN ME */}
      <div id="explain-section" className="py-20 scroll-mt-24">
        <div className="flex justify-center mb-5">
          <span className="text-[10px] font-bold tracking-[0.2em] text-white/40 uppercase bg-white/8 px-4 py-1.5 rounded-full border border-white/10">
            Explain Me
          </span>
        </div>
        <h2 className="text-center text-[28px] sm:text-[40px] lg:text-[48px] font-bold text-white mb-4 max-w-2xl mx-auto leading-[1.15]">
          Pick one topic to ask <br className="hidden sm:block" />and start from zero
        </h2>
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-2 gap-3 sm:gap-5 md:grid-cols-4 px-0 sm:px-4">
          {topics.map((topic) => (
            <Link href={topic.href} key={topic.title} className="group relative overflow-hidden rounded-2xl cursor-pointer block shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 active:scale-[0.98]" style={{ height: "clamp(160px, 30vw, 280px)" }}>
              <Image src={topic.image} alt={topic.title} fill sizes="(max-width: 640px) 50vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-110" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/15 to-transparent" />
              <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4">
                <h3 className="text-[13px] sm:text-base font-bold text-white text-center leading-tight drop-shadow-sm">{topic.title}</h3>
              </div>
            </Link>
          ))}
        </div>
      ...
      {/* QUIZ */}
      <div id="quiz-section" className="py-20 pb-32 scroll-mt-12">
        <div className="flex justify-center mb-5">
          <span className="text-[10px] font-bold tracking-[0.2em] text-white/40 uppercase bg-white/8 px-4 py-1.5 rounded-full border border-white/10">
            Quiz
          </span>
        </div>
        <h2 className="text-[28px] sm:text-[40px] lg:text-[48px] font-bold text-center text-white mb-3 leading-[1.15] max-w-xl mx-auto">
          Let&apos;s test your <br className="sm:hidden" />knowledge here!
        </h2>
        <p className="text-center text-white/50 text-[15px] mb-10">Select the level</p>

        {/* Cards row */}
        <div className="flex gap-4 sm:gap-6 justify-start sm:justify-center overflow-x-auto pb-4 px-1 snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
